...
const Canvas = ({
  ...
  const drawCanvasContent = () => {
    ...
    // 5. In drawCanvasContent, draw selection box and blue border for selected shapes
    // (add after drawing shapes)
    if (selectionBox) {
    ...
    // For selected shapes, draw a blue border (like hover)
    drawnRectangles.forEach((shape, i) => {
      if (selectedShapes.includes(i)) {
        ...
        if (shape.type === 'rectangle') {
          ...
          const measuredWidth = ctx.measureText(shape.text).width;
          const measuredHeight = (shape.fontSize || 16) * 1.2;
          ctx.strokeRect(shape.x, shape.y, measuredWidth, measuredHeight);
        }
        ctx.restore();
      }
    });
  ...
  // Re-draw canvas content when position, scale, drawnRectangles, hoveredShape, or activeTool changes
  useEffect(() => {
    drawCanvasContent();
  }, [
    position,
    ...
    hoveredShape,
    activeTool,
    selectionBox, // Added selectionBox to dependencies
    selectedShapes, // Added selectedShapes to dependencies
  ]);
