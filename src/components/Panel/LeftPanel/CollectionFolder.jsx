...
const CollectionFolder = ({
  ...
  const handleItemDrop = (e, dropIndex) => {
    ...
    setTimeout(() => {
      if (typeof window !== "undefined" && window.__DRAWN_RECTANGLES__) {
        console.log(
          "All drawnRectangles IDs:",
          window.__DRAWN_RECTANGLES__.map((s) => s.id)
        );
      }
    }, 100);
    setDragOverIndex(null);
  ...
  return (
    <div
      ...
      style={{
        // border: collectionOpenState.dragOverCollectionId === col.id ? "1px solid #1976d2" : undefined,
        borderRadius: 6,
      }}
    >
      <div
      ...
      >
        ...
        {renamingCollectionId === col.id ? (
          <input
            ...
            onKeyDown={(e) => {
              e.stopPropagation();
              handleCollectionRenameInputKeyDown(e, col.id);
            }}
            maxLength={32}
            onFocus={() =>
              console.log("Collection rename input focused for", col.id)
            }
          />
        ) : (
          col.name
